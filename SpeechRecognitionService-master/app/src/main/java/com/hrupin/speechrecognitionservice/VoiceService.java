...
public class VoiceService extends Service implements
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        //Set the keyphrase here by calling the get_keyword function from Contact class
        KEYPHRASE = intent.getStringExtra("keyword");
    ...
    public void onPartialResult(Hypothesis hypothesis) {
        if (hypothesis == null)
            return;
        //Where we check to see if the keyword was spoken
        String text = hypothesis.getHypstr();
        if (text.equals(KEYPHRASE)) {
            Toast.makeText(this, "onPartialResult text=" + text, Toast.LENGTH_SHORT).show();
            switchSearch(KWS_SEARCH);
            //Intent intent = new Intent(VoiceService.this, SendSMS.class);
    ...
    private void setupRecognizer(File assetsDir) throws IOException {
        ...
        recognizer = SpeechRecognizerSetup.defaultSetup()
                ...
                .setKeywordThreshold(Float.valueOf("1.e-" + 2 * sensibility)) // Threshold to tune for keyphrase to balance between false alarms and misses
